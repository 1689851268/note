...
用于克隆对象，克隆出来的对象的原型 `__proto__` 会指向被克隆的对象
...
console.log("per", per.name); // per superman
console.log("clonePer", clonePer.name); // clonePer superman

per.name = "monster";

console.log("per", per.name); // per monster
console.log("clonePer", clonePer.name); // clonePer monster
```

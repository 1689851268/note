...
数组可以看作特殊的对象（属性名为下标、属性值为数组元素，且有 length 属性的对象）。所以，`for … in` 语句还可以用于遍历数组，得到数组的下标，以获取数组的元素
...
for …of 语句会按照可迭代对象的 `next()` 方法产生值的顺序遍历迭代元素

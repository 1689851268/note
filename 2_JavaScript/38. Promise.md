...
- 回调函数 `executor`：`(resolve, reject) => {}`，也叫 "执行器"
  ...
  回调函数 `reject`：调用 `reject` 也会改变 Promise 实例的状态 pending → rejected
...
上例输出顺序：`before executor` → `executor` → `Promise {<fulfilled>: undefined}` → `after executor` 

- 如果要执行异步操作，一般会将异步操作放在 executor 内执行：
...
上例输出顺序：`before executor` → `executor` → `Promise {<fulfilled>: undefined}` →`after executor` → `setTimeout`
因为 “先执行同步操作，后执行异步操作”
...
1. 调用 `resolve`，Promise 实例的状态从 pending → fulfilled
2. 调用 `reject`，Promise 实例的状态从 pending → rejected
3. 抛出错误，Promise 实例的状态从 pending → rejected
...
- 状态只能从 pending → fulfilled / rejected，且状态一旦更新就不会再改变
...
- 如果参数 `value` 是 Promise 实例 → 直接返回该实例
...
- 参数 `reason` 为 Promise 实例的值；不传参数的话， Promise 实例的值为 `undefiend`
- 即使参数 `reason` 是 Promise 实例，也会成为新创建的 Promise 实例的值
...
2. 如果 `return` 的数据不是 Promise 实例，会自动将其转为 fulfilled 状态 Promise 实例；返回的数据即该 Promise 实例的值
...
setTimeout(() => {
    console.log("proThen", proThen); // proThen Promise {<rejected>: 'error'}
}, 0);
```
...
```js
let pro1 = new Promise(resolve => resolve()); // 同步改变状态

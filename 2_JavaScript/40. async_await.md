...
- `async` 修饰的函数为异步函数，异步函数需要返回 Promise 实例
...
let obj = {
    name: "superman",
    async showName() { // 对象方法
        let myName = await new Promise(resolve => {
...
1. 我们可以在 async 函数的**直接子作用域**中，使用 `await` 关键字来控制函数的执行
2. **async 函数中，第一个 await 之前的代码会同步执行，之后的代码会异步执行！！！**
...
async function show() {
    ...
    console.log("res", res); // reason wrong
}
show();
```

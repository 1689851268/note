...
- 通过用户的某些行为来触发函数，以实现某些操作

- DOM 的版本：0、1、2、3

- DOM1 没有添加事件内容；DOM3 只是在 DOM2 的基础上增加了事件类型
...
格式：**`dom.addEventListener('事件类型', 事件处理函数, 事件处理模型)`**
...
格式：**`dom.attachEvent('on事件类型', 事件处理函数)`**
...
function addEvent(dom, type, fun) {
    if (dom.addEventListener) { // 主流
        dom.addEventListener(type, fun, false);
    } else if (dom.attachEvent) { // IE8 及以下
        dom.attachEvent('on' + type, fun);
    } else { // DOM0
        dom['on' + type] = fun; // 等价于 “dom.on类型” 的写法
    }
}
...
> #### `dom.addEventListener('type', function, options)` 扩展用法
...
格式：**`dom.removeEventListener('事件类型', '同一个函数')`**

解绑不需要第 3 个参数

> #### IE8 及以下

格式：**`dom.detachEvent('on事件类型', '同一个函数')`**

> #### 兼容函数封装

```js
function removeEvent(dom, type, fun) {
    if (dom.removeEventListener) { // 主流
        dom.removeEventListener(type, fun);
    } else if (dom.detachEvent) { // IE8 及以下
        dom.detachEvent('on' + type, fun);
    } else { // DOM0
        dom['on' + type] = null;
    }
}
removeEvent(box, 'click', myClick);

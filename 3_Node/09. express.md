...
nodemon 用于监视 node.js 应用程序，如果有文件更新，nodemon 将自动重启 node 应用程序

1. 全局安装 nodemon 模块：`npm i -g nodemon` / `npm i -g nodemon -D`
2. 然后用 nodemon 运行服务端文件：`nodemon app.js`

- 这里可能报错，解决方法：以管理员身份运行 powerShell，输入 `set-ExecutionPolicy RemoteSigned` 并运行即可

...
3. 在路由处理函数的模块中导入：`const jwt = require("jsonwebtoken")`
4. 创建 config.js 文件，并向外共享 [加密] 和 [还原] token 的 jwtSecretKey 字符串

```js
module.exports = { jwtSecretKey: "mySecretKey" }
```

5. 将用户信息对象加密成 token 字符串（所以 token 字符串就是加密后的 `用户信息对象`）

```js
// 导入配置文件
const config = require("../config");
// 生成 token 字符串 - jwt.sign(用户信息对象, 加密密钥, 配置对象)
const tokenStr = jwt.sign(user, config.jwtSecretKey, { exporesIn: "10h" }); // 设置有效期为 10 小时
```
...
1. `npm i express-jwt`
2. 在 app.js 中配置 express-jwt 中间件（要在路由之前配置
...
3. 在 app.js 中的 [错误级别中间件] 中，捕获、处理 token 认证失败后的错误
...
- 此时，发送请求时，要在请求头的 **Authorization** 字段中加上 token 字符串，即加密后的用户信息对象
  这些信息被带到服务器后，会被解析、还原回原来的用户信息对象，并被挂载到 `req.user` / `req.auth` 中

...
打包工具 (如 Vite) 在编译时通过**依赖分析**来确定哪些资源需要打包, 动态拼接的路径在编译时无法被识别.

Vite 能处理:

1.  被明确导入的资源
2.  多媒体元素通过**静态链接**引用的资源 (eg: `<img src="xxx">`, `<video src="xxx">`, `<audio src="xxx">`)
3.  CSS 中通过**静态链接**引用的资源 (eg: `background-image: url(xxx)`)
4.  `import()` / `new URL()` 中通过**静态或半静态链接**引用的资源
...
1. 使用 `new URL()` 构造器创建 URL 对象 `url`
    1. 第 1 个参数传入资源的相对路径, 文件名部分支持动态
    2. 第 2 个参数传入 `import.meta.url` 作为基准 URL
2. 通过 `url.href` 属性获取完整的资源 URL

**工作原理**:

-   `new URL()` 是浏览器原生 Web API, 用于创建、解析和操作 URL
-   `import.meta.url` 提供当前模块的绝对 URL
...
静态资源动态加载问题的核心在于**构建工具的依赖分析机制**.

选择合适的方案需要综合考虑项目规模、资源数量、性能要求和开发维护成本.

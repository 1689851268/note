...
Vue 会加工、代理 `data` 为 `_data`，通过浏览器控制台可以观察到，`vm._data` 的属性都是响应式数据（有对应的 getter、setter）。但是，如果我们后期自己给 Vue 实例添加属性，该属性没有经过 Vue 加工，所以不是响应式数据（没有 getter、setter）。
...
如果我在 data 里面放置了一个对象数据，该**对象会被代理**，**对象里面的属性也会被代理**。就是说，不论我们修改对象本身，还是 [修改] 对象的属性，数据都是响应性的，会即时显示回页面。

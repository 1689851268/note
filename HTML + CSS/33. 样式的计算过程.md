...
浏览器会从各种来源的样式表中找到所有的 CSS 声明，并确定声明值。有 3 种源：

1.  用户代理样式表：浏览器内置的样式表。

2.  用户样式表：用户自定义主题时使用的样式表。
...
第一步：比较**源的重要性**。"作者样式表" > "用户样式表" > "用户代理样式表"
...
上例 `.test h1 { ... }` 的权重为 `(0, 1, 1, 1)`，而 `h1 { ... }` 的权重为 `(0, 0, 0, 1)`，因此最终会应用 `color: blue` 这个属性值。
...
-   背景系列属性：background、background-color、background-./pictures/image 等。

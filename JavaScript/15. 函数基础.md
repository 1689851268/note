...
向函数传递参数时，arguments 中对应的单元会和命名参数建立关联以得到相同的值。相反，不传递参数就不会建立关联。
...
函数重载是指在同一个作用域中定义多个具有**相同名称**但参数**个数或类型不同**的函数。JS 中并不直接支持函数重载，但可以通过一些技巧模拟函数重载的效果，实现相似的功能。**`arguments.length`** 可以返回类数组对象的长度。这样我们就可以根据**实参的数量**，来制定不同的功能代码。

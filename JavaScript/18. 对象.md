...
注意：属性名都是字符串类型的，所以使用字面量创建对象时，标准写法应该是 `{ 'name': 'superman' }`。但是，如果属性名符合标识符规范或为数字（数字会被 JS 隐式转为字符串），则可以不用引号包裹，反之 则必需加上引号。
...
`[]` 里面可以是任意返回字符串/数字的表达式（数字会被 JS 隐式转为字符串）：
...
Object 的原型对象身上有 4 个方法可以限制我们操作对象的属性。

...
事件中的 passive 属性是一个用于优化浏览器性能和用户体验的特性，它可以让开发者告诉浏览器，事件监听器中不会调用 `event.preventDefault()` 方法来阻止事件的默认行为。这样，浏览器就可以在两个线程里同时执行监听器中的 JS 代码和浏览器的默认行为，避免了等待和阻塞，提高了页面滑动的流畅度。

passive 属性的值是一个布尔值，true 表示监听器是顺从的，不会阻止默认行为，false 表示监听器是阻止的，可能会阻止默认行为。

passive 属性一般用在滚动、触摸等频繁触发的事件上，因为这些事件如果被阻止了，默认行为就会延迟或取消，导致页面卡顿或失去响应。使用 passive 属性可以让浏览器忽略 `event.preventDefault()` 方法，从而提升页面的性能和用户体验。

不过，有一些浏览器对于 passive 属性有一些特殊的处理。比如 Chrome 从版本 56 开始，默认将 touchstart 和 touchmove 的 passive 设置为 true，这样可以提高页面滑动的流畅度，但也可能导致一些兼容性问题。如果我们想要覆盖这个默认行为，就需要显式地将 passive 设置为 false。另外，有一些 CSS 属性也可以影响触摸事件的默认行为，比如 touch-action，它可以指定某个区域内是否允许用户操作，以及如何响应用户操作。

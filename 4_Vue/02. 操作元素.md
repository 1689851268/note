...
2.  `v-html`：`v-html` 指令值会替换掉标签元素里面的内容，且不会出现闪动问题；会解析内容中的 HTML 代码
...
因为 class 属性可以设置多个属性值，所以属性值可以为**对象** / **数组**
...
因为 style 属性可以设置多个属性值，所以属性值可以为**对象** / **数组**

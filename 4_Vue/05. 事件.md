...
`v-on:事件类型="方法名 / 表达式 / 简单 JS 语句"`，可以缩写为 `@事件类型="方法名 / 表达式 / 简单 JS 语句"`

* #### `@事件类型="方法名"`

```html
<div id="app">
    <button @click="show">点击打印数据</button>
</div>
```
...
let vm = new Vue({
    el: "#app",
    methods: {
        fun1() { console.log("first") },
        fun2() { console.log("second") }
    }
});
```
...
<input type="text" @keydown="showMsg">
```
...
可以看出，keyup 是按键抬起时触发、keydown 是按键按下时触发
...
*   `.ctrl`、`.alt`、`.shift`、`.meta`-windows键，这些键都需要配合 **keydown** 事件才能正常使用

    如果配合 keyup 使用：按下修饰键的同时，按下其他键，随后抬起其他键，才会触发事件
...
对于系统修饰符，如果配合 **keyup** 使用，需要按下其他键，随后抬起其他键，才会触发事件

```html
<!-- 按下 ctrl & enter 键才触发 -->
<input type="text" @keydown.ctrl.enter="showMsg">
<!-- 按下 ctrl & a 键才触发 -->
<input type="text" @keydown.ctrl.a="showMsg">
```

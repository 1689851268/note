...
路由，其实就是一些个 key-value 之间的映射

1. 后端路由：value 是 function，用于处理客户端提交的请求
   服务器接受到一个请求后，会根据**请求路径**找到匹配的**函数**来响应请求
2. 前端路由：value 是 component，用于展示页面内容
   根据路径的改变，显示、隐藏对应的组件
...
3. 在该文件中，设置路由规则，并创建 VueRouter 实例
...
- 可以给 router-link 设置 **active-class** 属性，设置其点击后的 class 类
- 不使用的路由，路由组件不会被创建
  离开当前路由，当前路由组件会被销毁，再次使用时再重新创建
- 使用路由后，VC 身上会多出两个属性 `$route` & `$router`
...
- 设置子路由时，如果该子路由的 `path` 属性值为空字符串，则为默认子路由
...
- `router.beforeEach((to, from, next) => {})`：全局前置守卫
  ...
  3. **`next`** - 放行函数；不调用则会一直卡在这里，无法执行后面的函数
     `next()`-放行全部路由
     `next(routerXXX)`-重定向到指定路由；`routerXXX` 可以为指定路由的路径 `/` / `{ path: "/" }` / `{ name: "routerName" }`
     `next(false)`-中断当前导航，重置到 `from` 的地址
...
10. 触发 DOM 更新
11. 调用 **<font style="color: yellow">组件守卫 `beforeRouteEnter(to, from, next) {}`** 中传给 `next` 的回调函数</font>，[创建好的组件实例] 会作为 [回调函数的参数] 传入

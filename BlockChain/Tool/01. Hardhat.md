...
Hardhat 是以太坊最流行的开发环境，它可以帮你编译和部署智能合约，并且内置了本地以太坊网络（Hardhat Node）支持本地测试和运行 Solidity。
...
2. 选择项目类型（本例选择 "Create a JavaScript project"）

3. 查看项目结构：

创建好的 Hardhat 项目主要包含以下文件和文件夹：
...
module.exports = {
    solidity: "0.8.24", // 编译器版本
};
```

这里的 "@NomicFoundation/hardhat-toolbox" 是一个非常有用的 Hardhat 插件包，集成了多个工具：

-   `hardhat-chai-matchers`：提供 Chai 断言工具，便于合约测试。

-   `hardhat-ethers`：集成 Ethers.js，方便在 Hardhat 中与以太坊合约交互。

-   `hardhat-ignition`：用于快速创建和测试合约项目。
...
编译成功后，你会在文件夹下看到 artifacts 目录，里面的 json 文件就是编译结果，包含了合约的 ABI 和字节码等信息。
...
```
  Lock
    Deployment
      ✔ Should set the right unlockTime (3696ms)
      ✔ Should set the right owner
      ✔ Should receive and store the funds to lock
      ✔ Should fail if the unlockTime is not in the future (50ms)
    Withdrawals
      Validations

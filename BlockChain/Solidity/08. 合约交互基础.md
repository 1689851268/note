...
接口（Interface）用于定义合约之间的交互标准，确保不同合约之间可以相互操作。

1.  接口不能定义状态变量
2.  接口只声明函数的签名，而不包含函数的实现；所有函数必须声明为 `external`
3.  接口不能包含构造函数
4.  接口可以继承其他接口，但不能继承合约
...
contract MyContract {
    ...
    function getCount(address _counter) public view returns (uint) {
        return Counter(_counter).count(); // 这里需使用 count 的 getter 获取值
    }
}
```

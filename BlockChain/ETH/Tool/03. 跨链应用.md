...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    WrappedNFT public wnft; // 定义 NFT 合约
    ...
    constructor(
        ...
        s_linkToken = IERC20(_link);
        wnft = WrappedNFT(wnftAddr); // 初始化 NFT 合约
    }
...
1.  安装依赖: `pnpm install -D @nomicfoundation/hardhat-ethers ethers hardhat-deploy hardhat-deploy-ethers @chainlink/env-enc`
...
module.exports = {
    ...
    networks: {
        sepolia: {
            ...
            chainId: 11155111,
            companionNetworks: { destChain: "amoy" },
        },
        amoy: {
            url: AMOY_RPC_URL,
            accounts: [PRIVATE_KEY],
            chainId: 80002,
            companionNetworks: { destChain: "sepolia" },
        },
    },
};
```
...
上述的 router linkToken companionChainSelector 等数据可从 [CCIP 支持的网络测试网 | Chainlink 文档](https://docs.chain.link/ccip/supported-networks/v1_2_0/testnet#ethereum-sepolia) 获取:

<img src="./picture/QQ_1728099711845.png" alt="QQ_1728099711845" style="zoom:25%;" />
...
# 输入 SEPOLIA_RPC_URL
# 输入 SEPOLIA_RPC_URL 的值 (Sepolia 即 Ethereum 的测试网)

# 输入 AMOY_RPC_URL
# 输入 AMOY_RPC_URL 的值 (Amoy 即 Polygon Pos 的测试网)
...
生成 .env.enc 文件, 存储着加密后的环境变量信息. 可以执行 `npx env-enc view` 查看配置的信息.

9.  执行 `npx hardhat deploy` 在本地部署合约, 检查是否能部署成功

<br><br>

# 单测

编写 test\unit\cross-chain-nft.test.js:
...
1.  `npx hardhat deploy --network sepolia --tags sourcechain`
2.  `npx hardhat deploy --network amoy --tags destchain`
...
发送 ccip 后, 可以通过 hash 到 [CCIP Explorer | Chainlink](https://ccip.chain.link/) 查看详细信息和交易进度.

4.  task\check-wrapped-nft.js:

...
可以使用 readonly 修饰数组类型，表示该数组为 [只读数组]。[只读数组] 不能新增、删除、更新数组的元素，也不能调用会修改原数组的方法：
...
TS 4.0 新增具名元组，用法一样，可读性更强：

```ts
let tuple1: [name: string, age: number];
```

元组类型会有一些小问题：

```ts
let tuple1: [name: string, age: number] = ["John", 30];
tuple1[2] = "Hello"; // Error: Tuple type '[string, number]' of length '2' has no element at index '2'.
tuple1.push("Hello"); // 这样新增元素居然不会报错hhh
```

我们可以使用 readonly 处理这个问题：

```ts
let tuple1: readonly [name: string, age: number] = ["John", 30];
tuple1.push("Hello"); // Error: Property 'push' does not exist on type 'readonly [name: string, age: number]'.
```
...
枚举类型编译成 JS 会成为一个立即执行函数 (IIFE)，IIFE 无法被 Tree Shaking 优化。
...
如果想在工程中使用枚举类型，需要设置 tsconfig.json 的 isolatedModules 为 true，因为有些构建工具并没有依赖 TS 的 tsc 进行类型检查和类型转译，像 esbuild、babel 会单独编译每个文件，因此它们无法判断导入的名称是类型还是值。所以一些 TS 特性容易产生错误，比如 const enum 。
...
let num = 1;
let str = "str";
num = str as number; // Conversion of type 'string' to type 'number' may be a mistake

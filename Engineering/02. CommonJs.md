...
1.  每个文件都是独立的模块。模块内的变量不在全局作用域中，模块存在自己的作用域，所以不能随意调用模块之间的变量。
2.  实际上，模块中的代码都包装在一个函数中，并且函数在执行时，接收了 5 个实参，可通过 `arguments` 查看：
...
1.  核心模块：`require('fs')`
2.  自定义模块：`require('./cjs1')`
    可以不写后缀，Node 会自动尝试补全文件后缀 → 补全 .js 进行加载 → 补全 .json 进行加载 → 补全 .node 进行加载
    前面的 `./` 不可以省略
3.  第三方模块：`require('mysql')`
    1.   查看是否为内置模块。若是，则直接使用；若不是，则往下走
    2.   到同级的 node_modules 目录中寻找 mysql 目录，找不到则往上一级的 node_modules 目录找，直至根目录。
    3.   在 mysql 目录下，找到 package.json 中的 `main` 字段，读取入口文件。
...
用于文件导出的属性是 `module.exports`，而 `exports` 是 `module.exports` 的引用。

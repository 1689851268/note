...
在依赖包的开发中，也需要注意 `devDependeces` 和 `dependences` 的配置，因为 npm 会认为 `dependences` 里边的依赖包为核心依赖包，而 `devDependences` 里边的依赖包为开发时态依赖。然后，当别人执行命令行 `npm i XXX` 安装你的依赖包时，核心依赖包会被一同安装，而开发时态依赖包则不会被安装。

<br>

在 CSR 开发中，执行 `npm i` 时，`dependencies`、`devDependencies` 指定的依赖包都会安装到 node_modules 目录下。

可以执行 `npm i --production` 仅安装 `dependecies` 指定的依赖包。
